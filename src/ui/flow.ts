...
export type SimNodeData = { simType: NodeType; name: string; seq: number; locked: boolean }
...
/** Board → React Flow state. Sequence numbers come from the names ("Web server 3" → 3) so new nodes don't collide. */
export function fromLevel(start: Graph): { nodes: FlowNode[]; edges: FlowEdge[] } {
  ...
  const nodes = start.nodes.map((n) => {
    const named = /(\d+)$/.exec(n.name)
    const seq = named ? Number(named[1]) : (counts[n.type] = (counts[n.type] ?? 0) + 1)
    return makeFlowNode(n.id, n.type, n.name, seq, { x: n.x, y: n.y }, n.locked)
  })
